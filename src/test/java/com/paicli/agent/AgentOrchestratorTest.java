...
class AgentOrchestratorTest {
    ...
    void shouldParseSimplePlan() {
        AgentOrchestrator orchestrator = new AgentOrchestrator(new OpenAiCompatibleClient("test-key"));
        String planJson = """
                {
                    "summary": "读取文件",
                    "steps": [
    ...
    @Test
    void reviewerUsesSeparateClientWhenProvided(@TempDir Path tempDir) {
        // 改进点 B：Worker 用 workerClient，Reviewer 用独立 reviewerClient。
        // worker 队列：计划 + 执行结果；reviewer 队列：审查通过。
        StubGLMClient workerClient = new StubGLMClient(List.of(
                response("""
                        {"summary":"单步","steps":[{"id":"s1","description":"执行任务","type":"COMMAND","dependencies":[]}]}
                        """),
                response("worker 执行结果")
        ));
        StubGLMClient reviewerClient = new StubGLMClient(List.of(
                response("""
                        {"approved": true, "summary": "通过", "issues": []}
                        """)
        ));
        ...
        String finalResult = orchestrator.run("测试 Reviewer 独立模型");

        // Reviewer 被独立 client 处理：reviewerClient 收到了审查请求（含执行结果）
        assertTrue(reviewerClient.capturedUserMessages.stream().anyMatch(m -> m.contains("worker 执行结果")),
                "Reviewer 应由独立 reviewerClient 处理");
        // worker 队列不该被审查消耗（reviewer 没去 poll workerClient）：worker 只被调了 2 次（计划+执行）
        assertEquals(2, workerClient.capturedUserMessages.size(), "workerClient 不应处理审查");
        assertTrue(finalResult.contains("worker 执行结果"));
    }

    @Test
    void reviewerFallsBackToWorkerClientWhenNull(@TempDir Path tempDir) {
        // reviewerClient=null（默认）→ Reviewer 复用 worker client，一个队列里依次：计划/执行/审查
        ...
        AgentOrchestrator orchestrator = new AgentOrchestrator(
                client, new ToolRegistry(), new NoOpMemoryManager(tempDir.toFile()),
                System.out, null);

        String finalResult = orchestrator.run("测试降级");
        // 一个 client 处理了全部 3 次（计划+执行+审查）→ 证明 null 时 Reviewer 复用了 worker client
        assertEquals(3, client.capturedUserMessages.size());
        assertTrue(finalResult.contains("执行结果"));
    }
    ...
    void shouldRetryRejectedStepUntilApproval(@TempDir Path tempDir) {
        ...
        // 改进点 A：重试时应把上一轮产出带回（previous_output），而非只给 issues。
        // 第二次 Worker 执行（捕获到的 user 消息里）应包含第一次的产出内容。
        boolean retryCarriedPreviousOutput = llmClient.capturedUserMessages.stream()
                .anyMatch(m -> m.contains("第一次执行结果") && m.contains("被审查拒绝"));
        assertTrue(retryCarriedPreviousOutput, "重试请求应带上一轮产出 + 拒绝说明");
    }
    ...
    private static final class NoOpMemoryManager extends MemoryManager {
        private NoOpMemoryManager(File storageDir) {
            super(new OpenAiCompatibleClient("test-key"), 32768, 200000, new LongTermMemory(storageDir));
        }
    }

    private static final class StubGLMClient extends OpenAiCompatibleClient {
        private final Queue<ChatResponse> responses;
        final List<String> capturedUserMessages = new java.util.ArrayList<>();
        ...
        public ChatResponse chat(List<Message> messages, List<Tool> tools, StreamListener listener) throws IOException {
            // 捕获本轮最后一条 user 消息内容，供断言重试时是否带上了上一轮产出
            for (int i = messages.size() - 1; i >= 0; i--) {
                if ("user".equals(messages.get(i).role())) {
                    String c = messages.get(i).content();
                    capturedUserMessages.add(c == null ? "" : c);
                    break;
                }
            }
     ...
     */
    private static final class DispatchingStubGLMClient extends OpenAiCompatibleClient {
