...
public class Agent {
    ...
    private boolean returnFinalResponseWhenStreamed = false;
    ...
    /**
     * 设置是否在流式输出完成后返回完整文本（wechat 通道需要）。
     */
    ...
    /**
     * 立即压缩对话历史（wechat 通道手动触发）。
     */
    public CompactionResult compactHistoryNow() {
        long beforeTokens = com.paicli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
        try {
            ...
            long afterTokens = com.paicli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
            return new CompactionResult(compacted, beforeTokens, afterTokens, null);
        } catch (Exception e) {
            log.warn("manual conversationHistory compaction failed", e);
            return new CompactionResult(false, beforeTokens, beforeTokens, e.getMessage());
        }
    }

    /**
     * 当前状态栏快照（wechat 通道状态展示）。
     */
    public StatusInfo currentStatus(String phase) {
        ...
        if ("idle".equals(normalizedPhase)) {
            return StatusInfo.idle(model, contextWindow, hitl);
        }
        return StatusInfo.active(model, contextWindow, hitl, normalizedPhase);
    }

    /**
     * 压缩结果（用于 wechat 手动压缩反馈）。
     */
    ...
    /**
     * 用已持久化的历史消息恢复当前会话（会话恢复 /resume /continue 用）。
     *
     * <p>保留当前 system 提示（第 0 条），用恢复的非 system 消息替换其余历史；
     * 恢复的 assistant.toolCalls / tool.toolCallId 原样保留，保证 OpenAI 协议配对正确。
     * 同步重建短期记忆（只灌 user/assistant 文本），避免 token 估算失准与记忆检索遗漏。
     */
    public void restoreConversationHistory(List<LlmClient.Message> restored) {
        ...
        conversationHistory.clear();
        conversationHistory.add(systemMsg);
        for (LlmClient.Message message : restored) {
        ...
        // 重建短期记忆，与 conversationHistory 对齐
        memoryManager.clearShortTerm();
        for (LlmClient.Message message : restored) {
            ...
            if ("user".equals(message.role())) {
                memoryManager.addUserMessage(message.content());
            } else if ("assistant".equals(message.role())) {
                memoryManager.addAssistantMessage(message.content());
            }
        }
        log.info("restored conversation history: {} messages", conversationHistory.size());
    }
    ...
    private String buildProjectMemoryContext() {
        try {
            return com.paicli.prompt.ProjectMemoryLoader
                    .createDefault(java.nio.file.Path.of(toolRegistry.getProjectPath()))
                    .loadForPrompt();
        } catch (Exception e) {
            log.warn("Failed to load project memory (PAI.md)", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        ...
        // 软提示 vs 硬限：agentTokenBudget(80%×window) 只是 UI 提示，不卡用户；真正能终止循环的是
        // AgentBudget 的硬 token 预算，默认不限。两者并排显示，避免软值被误读为强制上限。
        sb.append(String.format("  单轮 token 软提示: %s (80%% window)   硬限: %s%n",
                formatTokens(profile.agentTokenBudget()),
                com.paicli.agent.AgentBudget.hardTokenBudgetUnlimited()
                        ? "不限（默认）"
                        : formatTokens(com.paicli.agent.AgentBudget.configuredHardTokenBudget())));
    ...
    private String modelLabel() {
        ...
        return llmClient.getDisplayName();
    }
