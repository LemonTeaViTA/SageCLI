...
public class AgentBudget {
    ...
    /**
     * 当前生效的硬 token 预算（{@code paicli.react.token.budget}）；未显式配置则为 {@link Integer#MAX_VALUE}（不限）。
     * 供 /context 展示"软提示 vs 真硬限"用——让 ContextProfile.agentTokenBudget 那个 80% 软值不再被误读为强制上限。
     */
    public static int configuredHardTokenBudget() {
        return readIntProperty("paicli.react.token.budget", Integer.MAX_VALUE);
    }

    /** 硬 token 预算是否实质不限（默认即不限）。 */
    public static boolean hardTokenBudgetUnlimited() {
        return configuredHardTokenBudget() == Integer.MAX_VALUE;
    }
