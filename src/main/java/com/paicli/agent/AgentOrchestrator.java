...
public class AgentOrchestrator {
    ...
    // Reviewer 用的 LLM（可与 Worker 不同，降同模型盲点）。未指定时等于 llmClient。
    ...
    /**
     * @param reviewerLlmClient Reviewer 专用 LLM；传 null 则 Reviewer 复用 {@code llmClient}（默认行为，
     *                          不影响只有一个模型的用户）。配了不同模型时可降低"同模型 self-review"盲点。
     */
    public AgentOrchestrator(LlmClient llmClient, ToolRegistry toolRegistry,
                             MemoryManager memoryManager, PrintStream out, LlmClient reviewerLlmClient) {
        this.llmClient = llmClient;
        this.reviewerLlmClient = reviewerLlmClient != null ? reviewerLlmClient : llmClient;
        ...
        this.reviewer = new SubAgent("reviewer", AgentRole.REVIEWER, this.reviewerLlmClient, toolRegistry);
        ...
        for (ExecutionStep step : batch) {
            ...
            futures.add(executor.submit(() -> {
                ...
                SubAgent localReviewer = new SubAgent(
                        "reviewer-" + step.id(), AgentRole.REVIEWER, reviewerLlmClient, toolRegistry);
                try {
    ...
    private void runStep(ExecutionStep step, List<ExecutionStep> steps,
        ...
        while (!approved && retries < MAX_RETRIES_PER_STEP) {
            ...
            // 重试时把"上一轮的产出 + 审查问题"一起喂回去（对齐 wiki 三角色重试协议的
            // previous_output + issues）。Worker 每步 clearHistory 失忆，若只给 issues 不给上一版，
            // 它容易换方向重写而非针对性修补。上一版产出可能很长，截断防 user message 膨胀。
            String previousOutput = acceptedResult == null ? "" : acceptedResult;
            if (previousOutput.length() > 1000) {
                previousOutput = previousOutput.substring(0, 1000) + "...(已截断)";
            }
            String feedbackContext = context
                    + "\n\n你上一轮的执行结果如下，但被审查拒绝：\n" + previousOutput
                    + "\n\n审查指出的问题：\n" + issues
                    + "\n\n请针对上述问题修正你上一轮的结果，不要从头重写无关部分。";
