...
final class CliCommandParser {
    ...
    static ParsedCommand parse(String input) {
        ...
        if (trimmed.equalsIgnoreCase("/sessions") || trimmed.equalsIgnoreCase("/resume")) {
            // /resume 无参时也走列表（让用户先看到有哪些会话再挑）
            return new ParsedCommand(CommandType.SESSIONS_LIST, null);
        }
        ...
        if (trimmed.equalsIgnoreCase("/continue")) {
            return new ParsedCommand(CommandType.CONTINUE_SESSION, null);
        }
        ...
        if (trimmed.equalsIgnoreCase("/cost")) {
            return new ParsedCommand(CommandType.COST_STATUS, null);
        }
