...
final class PaiCliCompleter implements Completer {
    ...
    private final Supplier<List<ModelCatalog.ModelOption>> modelOptionSupplier;

    PaiCliCompleter(Supplier<List<McpResourceDescriptor>> resourceSupplier) {
        this(resourceSupplier, List::of, () -> ModelCatalog.builtins());
    }

    PaiCliCompleter(Supplier<List<McpResourceDescriptor>> resourceSupplier,
                    Supplier<List<Skill>> skillSupplier) {
        this(resourceSupplier, skillSupplier, () -> ModelCatalog.builtins());
    }

    PaiCliCompleter(Supplier<List<McpResourceDescriptor>> resourceSupplier,
                    Supplier<List<Skill>> skillSupplier,
                    Supplier<List<ModelCatalog.ModelOption>> modelOptionSupplier) {
        this.resourceSupplier = resourceSupplier;
        this.skillSupplier = skillSupplier == null ? List::of : skillSupplier;
        this.modelOptionSupplier = modelOptionSupplier == null ? () -> ModelCatalog.builtins() : modelOptionSupplier;
    }
    ...
    private boolean completeModel(String input, List<Candidate> candidates) {
        ...
        // 候选直接取 ModelCatalog 展示列表（以 config 为准、型号名、按系列分组）
        List<CommandOption> options = new ArrayList<>();
        for (ModelCatalog.ModelOption m : modelOptionSupplier.get()) {
            options.add(option(m.name(), m.description()));
        }
        addMatching(candidates, "模型", value, options.toArray(new CommandOption[0]));
        return true;
    }
